...
func (s *Store) SessionProfiles() []sessions.Profile {
	...
	for _, id := range s.sessionOrder { result = append(result, cloneProfile(s.sessionProfiles[id])) }
	return result
}
...
func (s *Store) UpsertSessionProfile(profile sessions.Profile) error {
	...
	if _, ok := s.sessionProfiles[profile.ID]; !ok { s.sessionOrder = append(s.sessionOrder, profile.ID) }
...
func (s *Store) DeleteSessionProfile(profileID string) error {
	s.mu.Lock(); defer s.mu.Unlock()
	if _, ok := s.sessionProfiles[profileID]; !ok { return fmt.Errorf("session profile %q not found", profileID) }
	...
	for _, id := range s.sessionOrder { if id != profileID { filtered = append(filtered, id) } }
...
func (s *Store) Settings() settings.AppSettings { s.mu.RLock(); defer s.mu.RUnlock(); return s.settings }

func (s *Store) WorkspaceLayout() workspace.Layout { s.mu.RLock(); defer s.mu.RUnlock(); return cloneWorkspaceLayout(s.workspaceLayout) }

func (s *Store) RuntimeTabs() []workspace.Tab {
	s.mu.RLock(); defer s.mu.RUnlock()
	result := make([]workspace.Tab, 0, len(s.runtimeOrder))
	for _, id := range s.runtimeOrder { result = append(result, s.runtimeTabs[id]) }
	return result
}
...
func (s *Store) Events() []workspace.Event {
	...
	for i := len(s.events)-1; i >= 0; i-- { result = append(result, s.events[i]) }
	return result
}

func (s *Store) OpenRuntimeTab(tab workspace.Tab) {
	s.mu.Lock(); defer s.mu.Unlock()
	if _, ok := s.runtimeTabs[tab.ID]; !ok { s.runtimeOrder = append(s.runtimeOrder, tab.ID) }
	s.runtimeTabs[tab.ID] = tab
	s.workspaceLayout.ActiveTabID = tab.ID
	s.events = append(s.events, workspace.Event{ID:s.nextEventIDLocked(), Type:"session.opened", Subject:tab.Title, At:time.Now().UTC().Format(time.RFC3339)})
}

func (s *Store) CloseRuntimeTab(sessionID string) bool {
	s.mu.Lock(); defer s.mu.Unlock()
	if _, ok := s.runtimeTabs[sessionID]; !ok { return false }
	delete(s.runtimeTabs, sessionID)
	removedIndex := -1
	filtered := make([]string, 0, len(s.runtimeOrder))
	for index, id := range s.runtimeOrder { if id == sessionID { removedIndex = index } else { filtered = append(filtered, id) } }
	if removedIndex >= 0 { s.events = append(s.events, workspace.Event{ID:s.nextEventIDLocked(), Type:"session.closed", Subject:sessionID, At:time.Now().UTC().Format(time.RFC3339)}) }
	s.runtimeOrder = filtered
	if len(filtered) == 0 { s.workspaceLayout.ActiveTabID = "" } else { next := removedIndex; if next >= len(filtered) { next = len(filtered)-1 }; if next < 0 { next = 0 }; s.workspaceLayout.ActiveTabID = filtered[next] }
	return true
}

func (s *Store) RecordLaunch(profileID string) {
	s.mu.Lock(); defer s.mu.Unlock()
	profile, ok := s.sessionProfiles[profileID]; if !ok { return }
	...
	if len(s.launchHistory) > maxLaunchHistoryEntries { s.launchHistory = append([]sessions.HistoryEntry(nil), s.launchHistory[len(s.launchHistory)-maxLaunchHistoryEntries:]...) }
}

func (s *Store) nextEventIDLocked() string { s.eventCounter++; return fmt.Sprintf("event-%d", s.eventCounter) }

func (s *Store) LockSecureStorage() { s.mu.Lock(); defer s.mu.Unlock(); s.secureStatus.Unlocked = false }
func (s *Store) SecureStorageStatus() securestorage.Status { s.mu.RLock(); defer s.mu.RUnlock(); return s.secureStatus }

func (s *Store) EnsureMasterPassword(password string) error {
	s.mu.Lock(); defer s.mu.Unlock()
	if len(password) == 0 { return fmt.Errorf("master password is required") }
	s.secureStatus = securestorage.Status{Available:true, Configured:true, Unlocked:true}
	return nil
}

func (s *Store) SecretExists(key string) bool { s.mu.RLock(); defer s.mu.RUnlock(); return s.secretExistsLocked(key) }
func (s *Store) LoadSecret(key string) (string, error) { s.mu.RLock(); defer s.mu.RUnlock(); return s.secrets[key], nil }
func (s *Store) StoreSecret(key, value string) error { s.mu.Lock(); defer s.mu.Unlock(); s.secrets[key] = value; return nil }
func (s *Store) DeleteSecret(key string) error { s.mu.Lock(); defer s.mu.Unlock(); delete(s.secrets, key); return nil }
func (s *Store) secretExistsLocked(key string) bool { _, ok := s.secrets[key]; return ok }
...
func defaultCommandPolicy() ai.CommandPolicy {
	return ai.CommandPolicy{
		Tools: []ai.CommandTool{{ID:"shell", Name:"Shell command", Description:"Run command in active SSH session", Enabled:true}, {ID:"sftp", Name:"SFTP operations", Description:"Browse and edit files over SFTP", Enabled:true}, {ID:"search", Name:"Search", Description:"Run grep/find-like queries on host", Enabled:true}},
		AllowedTools: []string{}, SessionAllowedTools: map[string][]string{}, PendingRequests: []ai.CommandRequest{},
	}
}

func defaultSettings() settings.AppSettings {
	return settings.AppSettings{Theme:"dark", DefaultProtocol:"ssh", WindowLayout:settings.WindowLayout{SidebarWidth:300, AssistantWidth:360}, PromptBeforeAI:true, AllowCloudModels:true, SSHConfigAutoLoaded:false, VaultMountPoint:settings.DefaultVaultMountPoint, VaultAutoRenewToken:false, VaultAuthMethod:settings.DefaultVaultAuthMethod, VaultProvider:settings.DefaultVaultProvider}
}

func defaultWorkspaceLayout() workspace.Layout { return workspace.Layout{SidebarSections:[]workspace.SidebarSection{{ID:"sessions", Title:"Sessions"}, {ID:"files", Title:"SFTP Browser"}}} }
...
func cloneCommandPolicy(policy ai.CommandPolicy) ai.CommandPolicy {
	...
	if policy.SessionAllowedTools != nil { cloned.SessionAllowedTools = make(map[string][]string, len(policy.SessionAllowedTools)); for id, tools := range policy.SessionAllowedTools { cloned.SessionAllowedTools[id] = append([]string(nil), tools...) } } else { cloned.SessionAllowedTools = map[string][]string{} }
	return cloned
}

func cloneWorkspaceLayout(layout workspace.Layout) workspace.Layout { cloned := layout; cloned.SidebarSections = append([]workspace.SidebarSection(nil), layout.SidebarSections...); return cloned }

func cloneProfile(profile sessions.Profile) sessions.Profile {
	cloned := profile
	cloned.Tags = append([]string(nil), profile.Tags...)
	if profile.Options != nil { cloned.Options = make(map[string]string, len(profile.Options)); for key, value := range profile.Options { cloned.Options[key] = value } }
	return cloned
}
