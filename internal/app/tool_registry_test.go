...
func TestCommandToolRegistry(t *testing.T) {
	...
	if len(tools) != 6 {
		t.Fatalf("expected six AI tools, got %d", len(tools))
	}
	if tools[0].Name != "ssh.exec" || tools[1].Name != "sftp.write" || tools[2].Name != "sftp.stat" || tools[3].Name != "sftp.read" || tools[4].Name != "sftp.list" || tools[5].Name != "ssh.diagnostics" {
	...
	diagnosticsParams, ok := tools[5].Parameters["properties"].(map[string]any)
	if !ok {
...
func TestOpenAIToolDefinitionsAreValidJSON(t *testing.T) {
	...
	if len(decoded) != 6 || decoded[0]["type"] != "function" || decoded[1]["type"] != "function" || decoded[2]["type"] != "function" || decoded[3]["type"] != "function" || decoded[4]["type"] != "function" || decoded[5]["type"] != "function" {
