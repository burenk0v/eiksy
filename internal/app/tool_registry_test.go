...
func TestCommandToolRegistry(t *testing.T) {
	...
	if tools[0].Name != "ssh.exec" || tools[1].Name != "sftp.write" || tools[2].Name != "sftp.read" || tools[3].Name != "sftp.list" || tools[4].Name != "ssh.diagnostics" {
	...
	diagnosticsParams, ok := tools[4].Parameters["properties"].(map[string]any)
	if !ok {
...
func TestOpenAIToolDefinitionsAreValidJSON(t *testing.T) {
	...
	if len(decoded) != 5 || decoded[0]["type"] != "function" || decoded[1]["type"] != "function" || decoded[2]["type"] != "function" || decoded[3]["type"] != "function" || decoded[4]["type"] != "function" {
