...
func TestNativeToolApprovalResumesConversation(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		for _, message := range request.Messages {
			if message.Role == "tool" && message.ToolCallID == "call-approval" && strings.Contains(message.Content, `"status":"executed"`) {
				foundToolResult = true
			}
		}
		if !foundToolResult {
			t.Fatalf("expected executed tool result in continuation: %#v", request.Messages)
		}
	...
	if pending.ToolCallID != "call-approval" {
		t.Fatalf("unexpected pending tool call id: %q", pending.ToolCallID)
	}

	if err := service.ResolveCommandPolicyRequest(pending.RequestID, ai.CommandPermissionModeNow); err != nil {
	...
	if finalState.PendingNativeToolCall != nil {
		t.Fatal("expected pending native tool call to be cleared")
	}
	if len(finalState.CommandPolicy.PendingRequests) != 0 {
		t.Fatalf("expected pending requests to be cleared, got %d", len(finalState.CommandPolicy.PendingRequests))
	}
	if len(finalState.Messages) != 2 || finalState.Messages[0].Role != "user" || finalState.Messages[1].Content != "Command completed successfully." {
		t.Fatalf("unexpected final chat messages: %#v", finalState.Messages)
	}
}
