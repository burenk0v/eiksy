...
func TestDownloadAndStartLocalModel(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) { _, _ = w.Write([]byte("gguf")) }))
	defer server.Close()
	binDir := t.TempDir()
...
func TestStartCloudProviderAuthBuildsSourcegraphCallbackURL(t *testing.T) {
	...
	if authURL.Query().Get("state") == "" {
		t.Fatal("expected authorization callback state")
	}
}
...
func TestCloudProviderAuthSessionCompletesFromLocalhostCallback(t *testing.T) {
	...
	resp, err := http.Post("http://localhost:"+port+"/api/sourcegraph/token?state="+url.QueryEscape(authURL.Query().Get("state")), "application/json", bytes.NewBufferString(`{"token":"browser-token"}`))
	if err != nil {
...
func TestCloudProviderAuthSessionAcceptsPostedAccessToken(t *testing.T) {
	...
	resp, err := http.Post("http://127.0.0.1:"+port+"/api/sourcegraph/token?state="+url.QueryEscape(authURL.Query().Get("state")), "application/json", bytes.NewBufferString(`{"accessToken":"posted-token"}`))
	if err != nil {
...
func TestResolveCommandPolicyRequestWithSessionApprovalExecutesCommand(t *testing.T) {
	store := memory.NewStore()
	profile := sessions.Profile{ID: "ssh-host", Name: "ssh-host", ProtocolID: "ssh", Host: "host", Port: 22, Username: "ops"}
	...
	state := store.AIState()
	state.CommandPolicy.PendingRequests = []ai.CommandRequest{{ID: "request-1", ToolID: "shell", SessionID: tab.ID, Command: "uname -a"}}
	store.UpdateAIState(state)
	if err := service.ResolveCommandPolicyRequest("request-1", ai.CommandPermissionModeSession); err != nil {
		t.Fatalf("resolve request: %v", err)
	}
	updated := store.AIState()
	if len(updated.CommandPolicy.PendingRequests) != 0 {
		t.Fatalf("expected pending requests to be cleared, got %d", len(updated.CommandPolicy.PendingRequests))
	}
	if !hasCommandRule(updated.CommandPolicy.CommandRules, "shell", tab.ID, "uname -a", ai.CommandPermissionAllow) {
		t.Fatalf("expected exact shell command grant for session %s", tab.ID)
	}
	if len(ssh.inputs) != 1 {
		t.Fatalf("expected exactly one command dispatch, got %d", len(ssh.inputs))
	}
	if ssh.inputs[0].tabID != tab.ID {
		t.Fatalf("expected command to run in tab %s, got %s", tab.ID, ssh.inputs[0].tabID)
	}
	if ssh.inputs[0].payload != "uname -a\n" {
...
func TestUpdateCommandPolicyNormalizesState(t *testing.T) {
	...
	if err := service.UpdateCommandPolicy(policy); err != nil {
		t.Fatalf("update command policy: %v", err)
	}
	state := service.GetShellState()
	if state.AI.CommandPolicy.LocalDocsPath != "/tmp/docs" {
		t.Fatalf("expected docs path to be trimmed, got %q", state.AI.CommandPolicy.LocalDocsPath)
	}
	if !slices.Contains(state.AI.CommandPolicy.AllowedTools, "shell") {
		t.Fatal("expected shell in normalized allowed tools")
	}
...
func TestResolveCommandPolicyRequestPersistsAlwaysGrantOnExecutionFailure(t *testing.T) {
	...
	state := store.AIState()
	state.CommandPolicy.PendingRequests = []ai.CommandRequest{{ID: "request-fail", ToolID: "shell", SessionID: tab.ID, Command: "hostname"}}
	store.UpdateAIState(state)
	...
	if !hasCommandRule(updated.CommandPolicy.CommandRules, "shell", "", "hostname", ai.CommandPermissionAllow) {
		t.Fatalf("expected exact global command grant to persist, got %+v", updated.CommandPolicy.CommandRules)
	}
}
...
func TestUpdateSettingsVaultPasswordPersistsOnBlankUpdate(t *testing.T) {
	...
	if err := store.StoreSecret(securestorage.VaultPasswordKey(), "vault-secret"); err != nil {
		t.Fatalf("store vault password: %v", err)
	}
	if err := store.UpdateSettings(initial); err != nil {
		t.Fatalf("seed settings: %v", err)
	}
	...
	reloaded := store.Settings()
	if !reloaded.HasVaultPassword {
		t.Fatal("expected vault password flag to persist")
	}
	password, err := store.LoadSecret(securestorage.VaultPasswordKey())
	if err != nil {
		t.Fatalf("load vault password: %v", err)
	}
	if password != "vault-secret" {
		t.Fatalf("expected vault password to persist, got %q", password)
	}
}

func TestApplySSHForwardingSettingsUsesRemoteHostAndPort(t *testing.T) {
	store := memory.NewStore()
	target := sessions.Profile{ID: "jump-host", Name: "jump-host", ProtocolID: "ssh", Host: "jump.internal", Port: 22, Username: "ops"}
	source := sessions.Profile{ID: "source-host", Name: "source-host", ProtocolID: "ssh", Host: "source.internal", Port: 22, Username: "ops"}
	...
	cfg := store.Settings()
	cfg.PortForwardRules = []settings.PortForwardRule{{LocalPort: "15432", RemoteHost: "db.internal", RemotePort: "5432", HostID: "jump-host", Enabled: true}}
